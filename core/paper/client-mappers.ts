...
function normalizeActivityTitle(row: Record<string, unknown>) {
  ...
  if (title === "Paper buy executed" || type === "trade.opened") return "Position opened";
  if (title === "Paper position closed" || type === "trade.closed") return "Position closed";
  if (title === "Paper position reduced" || title === "Position reduced" || type === "position.reduced") {
    return "Position partially reduced";
  }
  if (type === "position.increased") return "Position increased";
  if (type === "position.opened.long") return "Long position opened";
  if (type === "position.opened.short") return "Short position opened";
  if (type === "position.increased.long") return "Long position increased";
  if (type === "position.increased.short") return "Short position increased";
  if (type === "position.levels.updated.stop_loss") return "Stop loss updated";
  if (type === "position.levels.updated.take_profit") return "Take profit updated";

  return title;
}

export function mapWorkspaceIdentityRow(row: Record<string, unknown>): WorkspaceIdentity {
  return {
    id: String(row.id),
...
export function mapPaperPositionRow(row: Record<string, unknown>): PaperPosition {
  return {
    id: String(row.id),
...
export function mapPaperTradeRow(row: Record<string, unknown>): PaperTrade {
  const action = String(row.action ?? row.side ?? "").toLowerCase();
  return {
    id: String(row.id),
    productId: String(row.product_id),
    symbol: String(row.symbol),
    action: action === "close" ? "close" : action === "increase" ? "increase" : "open",
    quantity: toNumber(row.quantity),
...
export function mapWorkspaceActivityRow(row: Record<string, unknown>): WorkspaceActivityItem {
  return {
    id: String(row.id),
