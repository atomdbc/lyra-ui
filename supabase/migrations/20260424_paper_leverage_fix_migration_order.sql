-- Re-apply 40× paper leverage after filename ordering caused an older migration to win.
-- On fresh applies, `20260402_leverage_max_40x.sql` runs before `20260402_leverage_support.sql`
-- (lexicographic: "leverage_m..." < "leverage_s..."), so `leverage_support.sql` overwrote the
-- CHECK constraint and `lyra_open_or_increase_paper_position` back to 1–3× only.
-- This migration restores the intended app/Postgres contract (1–40, env-capped in the app).
